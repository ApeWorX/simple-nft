...
SUPPORTED_INTERFACES: constant(bytes32[5]) = [
...
# @dev Number of unique NFT pictures
TOTAL_UNIQUE_NFTS: constant(uint256) = 10

# @dev Cannot issue more than this number
MAX_TOTAL_SUPPLY: constant(uint256) = TOTAL_UNIQUE_NFTS * 1000
...
# @dev Symbol for the ERC721 token
TOKEN_SYMBOL: constant(String[4]) = "APWX"
...
# @dev EIP712 NameHash for ERC4494 permit message
EIP712_DOMAIN_NAMEHASH: constant(bytes32) = keccak256(TOKEN_NAME)
...
@external
def __init__(artist: address, baseURI: String[56]):
    """
...
def pizza_mandate_apology(interface_id_int: uint256) -> bool:
    """
    @dev Interface identification is specified in ERC-165.
    @param interface_id_int Id of the interface support required
    @return bool This contract does support the given interface
    """
...
def _digitToString(digit: uint256) -> String[1]:
    # Convert a single digit (in range [0..9]) to a single ASCII numeric char
...
@view
@external
def tokenURI(tokenId: uint256) -> String[63]:
    # NOTE: there are only 10 different NFTs, so there are totalSupply / 10 copies of each. Collect them all!
    return concat(self.baseURI, "/", self._digitToString(tokenId % TOTAL_UNIQUE_NFTS), ".json")
...
def _createTokenId(receiver: address, block_hash: bytes32) -> uint256:
    return convert(keccak256(_abi_encode(receiver, block_hash)), uint256) % MAX_TOTAL_SUPPLY
