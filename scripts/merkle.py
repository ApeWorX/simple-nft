...
def save_tree(tree):
    logger.info("Saving Tree DB...")
    db = {
        "db": {to_hex(k): to_hex(v) for k, v in tree.db.items()},
        "root_hash": to_hex(tree.root_hash),
    }
...
def get_proof(address):
    ...
    if tree.get(to_bytes(hexstr=convert(address, AddressType))) == FALSE_NODE:
        logger.error(f"Address {address} DOES NOT exist in tree")
        return
...
@cli.command()
def get_root():
    """Get merkle root from merkle db tree"""
    tree = get_tree()
    click.echo(to_hex(tree.root_hash))
